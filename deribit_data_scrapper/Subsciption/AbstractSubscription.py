...
class AbstractSubscription(ABC):
    ...
    def __init__(self, scrapper: scrapper_typing, request_typo: RequestTypo):
        ...
        self.client_id = (
            ...
            if self.scrapper.configuration["orderBookScrapper"]["test_net"]
            else self.scrapper.configuration["user_data"]["production"]["client_id"]
        )

        self.client_secret = (
            self.scrapper.configuration["user_data"]["test_net"]["client_secret"]
            if self.scrapper.configuration["orderBookScrapper"]["test_net"]
            else self.scrapper.configuration["user_data"]["production"]["client_secret"]
        )
    ...
    def record_to_database(
        self, record_dataframe: DataFrame, tag_of_data: str = None
    ) -> DataFrame:
        """
        ...
        :return:
        """
        return self._record_to_daemon_database_pipeline(
            record_dataframe=record_dataframe.copy(), tag_of_data=tag_of_data
        )
