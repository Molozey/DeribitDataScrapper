...
@nb.njit()
def implied_vol(
    V_mkt, S0, K, T, r, is_call=True, tol=10**-4, sigma_l=10**-8, sigma_r=2
):
    if (
        g_impvol(V_mkt, sigma_l, T, S0, K, r, is_call)
        * g_impvol(V_mkt, sigma_r, T, S0, K, r, is_call)
        > 0
    ):
...
def get_implied_vols(forward, maturity, strikes, forward_values, is_call=True):
    ...
    for index in range(n):
        ...
        ivols[index] = implied_vol(V_mkt, f, K, T, r=0.0, is_call=is_call)
    return ivols
...
def vol_sabr(forward, maturity, strikes, backbone, params, compute_risk=False):
    ...
    if compute_risk:
        dsigma_dalphas, dsigma_drhos, dsigma_dvs = jacobian_sabr(
            forward, maturity, strikes, backbone, params
        )
    ...
    for index in range(n):
        ...
        if compute_risk:
            ...
            vega_bsm = black_scholes_vega(sigma, f, K, T, r=0.0)
            delta_bsm = black_scholes_delta(sigma, f, K, T, r=0.0, is_call=True)
            ...
            # todo: SABR gamma
            gammas[index] = black_scholes_gamma(sigma, f, K, T, r=0.0)
...
def calibrate_sabr(
    ...
    def get_residuals(params):
        J = np.stack(jacobian_sabr(forward, maturity, strikes, backbone, params))
        iv, _ = vol_sabr(
            forward, maturity, strikes, backbone, params, compute_risk=False
        )
...
class SabrCalibration(AbstractExternal, Thread):
    ...
    def main(self):
        ...
        app.layout = html.Div(
            [
                ...
                dcc.Interval(
                    id="interval", interval=20  # in millisecond 1*1000= 1 second
                ),
            ]
        )
        global web_app
        ...
        @app.callback(
            Output("live-graph-index", "figure"), [Input("interval", "n_intervals")]
        )
        def update_line_chart(self):
            _real_implied_vols_fig = go.Scatter(
                x=web_app.test_x_values, y=web_app.test_y_values
            )
    ...
    def collect_data_from_instruments(self):
        ...
        instruments_maturities = np.array(
            [
                instrument.get_raw_instrument_maturity()
                for instrument in recorded_instruments
            ]
        )
        instruments_maturities = instruments_maturities[
            instruments_maturities != np.array(None)
        ]
        ...
        for maturity in unique_maturities:
            ...
            extract_future = list(
                filter(
                    lambda _: _.instrument_type == InstrumentType.FUTURE,
                    available_instruments,
                )
            )[0]
            ...
            call_options = list(
                filter(
                    lambda _: _.instrument_type == InstrumentType.CALL_OPTION,
                    available_instruments,
                )
            )
            put_options = list(
                filter(
                    lambda _: _.instrument_type == InstrumentType.PUT_OPTION,
                    available_instruments,
                )
            )
    ...
    def test_calculation(self, put_strikes, call_strikes, T, forward, puts, calls):
        ...
        calibrated_params, error = calibrate_sabr(
            forward, T, strikes, implied_vols, beta
        )

        test_strikes = np.linspace(strikes[0], strikes[-1], 100)
        test_iv, _ = vol_sabr(
            forward, T, test_strikes, beta, calibrated_params, compute_risk=False
        )
