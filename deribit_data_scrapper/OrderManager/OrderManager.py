...
class OrderManager(ABC):
    ...
    def __init__(self):
        ...
        self.used_tags = CircularBuffer(
            size=300
        )  # TODO: need to be tested. Probably may be to small
    ...
    async def _extract_order_callback(self, callback: dict):
        ...
        if "params" in callback:
            ...
            if (not _callback_data["api"]) and self.process_only_API_orders:
                logging.error(
                    "Order created by hand trading (by deribit.com). OrderManager will no process this right now. Need to be implemented"
                )
        ...
        (
            ...
            _order_type,
            _order_state,
        ) = await self._extract_values_from_callback(
            _callback_data
        )  # Extract values from callback
        _order = await self.collect_order_object_by_tag(
            order_tag=_tag
        )  # Collect order object by tag
        if type(_order) == OrderStructure:
            await self._if_order_exist(
                orderStructure=_order,
                _order_tag=_order_tag,
                _price=_price,
                _executed_price=_executed_price,
                _total_commission=_total_commission,
                _order_amount=_order_amount,
                _filled_amount=_filled_amount,
                _last_update_time=_last_update_time,
                _order_state=_order_state,
                _order_exist_time=_order_exist_time,
    ...
    async def _if_order_exist(
        ...
        await self.client.instrument_manager.change_user_instrument_position(
            position_change=_filled_amount - _prev_filled,
            instrument_name=orderStructure.instrument,
            increment=True,
        )
        ...
        if orderStructure.order_state != _order_state:
            await self.change_order_state(
                order_tag=_order_tag, newOrderState=_order_state
            )

        await self.client.connected_strategy.on_order_update(
            await self.collect_order_object_by_tag(order_tag=_order_tag)
        )

    async def _if_order_dont_exist(
        self,
        order_tag: str,
        order_id,
        open_time,
        ...
        # Change user position
        await self.client.instrument_manager.change_user_instrument_position(
            position_change=filled_amount - _prev_filled,
            instrument_name=_new_order.instrument,
            increment=True,
        )

        await self.client.connected_strategy.on_order_creation(_new_order)

    async def collect_order_object_by_tag(
        self, order_tag: str
    ) -> Union[OrderStructure, int]:
        """
    ...
    async def change_order_state(self, order_tag: str, newOrderState: OrderState):
        ...
        if order_tag in self.open_orders:
            ...
            raise MemoryError(
                "Existed Order Tag doesn't have match in Order storage structures"
            )
    ...
    async def _extract_values_from_callback(self, _callback_data):
        ...
        _order_type = convert_deribit_order_type_to_structure(
            _callback_data["order_type"]
        )
        _order_state = convert_deribit_order_status_to_structure(
            _callback_data["order_state"]
        )
        return (
            _order_id,
            _order_tag,
