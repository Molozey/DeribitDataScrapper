...
def test_message() -> dict:
    _msg = {"jsonrpc": "2.0", "id": 8212, "method": "public/test", "params": {}}
    return _msg
...
def make_subscription_all_book(
    instrument_name: str, type_of_data="book", interval="100ms"
) -> dict:
...
def make_subscription_constant_book_depth(
    instrument_name: str, type_of_data="book", interval="100ms", depth=None, group=None
) -> dict:
    if not depth:
        ...
        if not group:
            ...
            warnings.warn(
                "Highly recommended not to use group. It can be unstable right now"
            )
...
def make_trades_subscription_request_by_currency(
    currency: AvailableCurrencies.Currency, kind="option", interval="100ms"
):
    ...
    _msg = {
        ...
        "id": 42,
        "params": {"channels": [f"trades.{kind}.{currency.currency}.{interval}"]},
    }
    return _msg


def make_trades_subscription_request_by_instrument(
    instrument_name: str, interval="100ms"
):
    _msg = {
        "jsonrpc": "2.0",
        "method": "public/subscribe",
        "id": 42,
        "params": {"channels": [f"trades.{instrument_name}.{interval}"]},
    }
    return _msg
...
def make_user_orders_subscription_request_by_instrument(instrument_name: str):
    _msg = {
        ...
        "id": 42,
        "params": {"channels": [f"user.orders.{instrument_name}.raw"]},
    }
    return _msg
...
def get_ticker_by_instrument_request(instrument_request: str) -> dict:
    _msg = {
        ...
        "params": {"instrument_name": f"{instrument_request}"},
    }
    return _msg
...
def get_user_portfolio_request(currency: AvailableCurrencies.Currency) -> dict:
    _msg = {
        ...
        "method": "private/subscribe",
        "id": 42,
        "params": {"channels": [f"user.portfolio.{currency.currency.lower()}"]},
    }
...
def get_positions_request(currency: AvailableCurrencies.Currency, kind="future"):
    msg = {
        ...
        "params": {"currency": f"{currency.currency}", "kind": f"{kind}"},
    }
    return msg
...
def get_instrument_position_request(instrument_name: str):
    msg = {
        ...
        "params": {"instrument_name": f"{instrument_name}"},
    }
    return msg
