...
def send_batch_of_requests(msg_list, show_answer=False) -> json:
    ...
    async def circle(
        mutable_list,
        iteration_of_request,
        one_request_size=_one_request_batch_size,
        load_mod=False,
    ):
        if not load_mod:
            messages = [
                ...
                for msg in msg_list[
                    iteration_of_request
                    * one_request_size : (iteration_of_request + 1)
                    * one_request_size
                ]
            ]
        else:
            messages = [
                call_api(json.dumps(msg))
                for msg in msg_list[iteration_of_request * one_request_size :]
            ]
    ...
    if len(msg_list) >= _one_request_batch_size:
        for iteration_number in tqdm(
            range(0, len(msg_list) // _one_request_batch_size)
        ):
            asyncio.run(circle(return_list, iteration_of_request=iteration_number))
            time.sleep(2)
        if len(return_list) != len(msg_list):
            print(
                f"Need to load mod part of map | Downloaded size {len(return_list)} | Message size {len(msg_list)}"
            )
            asyncio.run(
                circle(
                    return_list,
                    iteration_of_request=len(msg_list) // _one_request_batch_size,
                )
            )

    else:
        asyncio.run(
            circle(return_list, iteration_of_request=0, one_request_size=len(msg_list))
        )
        time.sleep(2)
...
def test_get_multiple_ticker_request():
    names = [
        "BTC-29SEP23-10000-C",
        "BTC-30DEC22-10000-C",
        "BTC-30JUN23-10000-C" "BTC-31MAR23-10000-C",
        "BTC-25NOV22-12000-C",
        "BTC-25NOV22-14000-C",
        "BTC-28OCT22-14000-C",
        "BTC-29SEP23-14000-C",
