...
async def scrap_available_instruments_by_extended_config(currency: Currency, cfg):
    ...
    if not cfg["use_configuration_to_select_maturities"]:
        ...
        if selected_maturity == -1:
            ...
            return []
        # selected_maturity = 3
        selected_maturity = available_maturities.iloc[selected_maturity][
            "DeribitNaming"
        ]
        ...
        try:
            with open(
                ...
                + cfg["maturities_configuration_path"],
                "r",
            ) as ymlfile:
                maturities_selected = yaml.load(ymlfile, Loader=yaml.FullLoader)[
                    "maturities_list"
                ]
        ...
        for maturity in maturities_selected:
            selected = list(
                map(
                    ...
                    list(
                        filter(
                            lambda x: (maturity in x["instrument_name"])
                            and (x["option_type"] == "call" or "put"),
                            answer,
                        )
                    ),
