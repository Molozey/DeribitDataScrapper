...
async def scrap_all_instruments_from_currency(currency: Currency, cfg):
    ...
    """
    from deribit_data_scrapper.SyncLib.AvailableRequests import (
    ...
    all_available_underlying = await send_request(
        get_instruments_by_currency_request(
            currency=currency, kind=InstrumentType.FUTURE, expired=False
        )
    )
    ...
    available_maturities_underlying = np.unique(
        list(map(lambda x: x["instrument_name"].split("-")[1], answer_underlying))
    )
    ...
    available_maturities_underlying["RealNaming"] = pd.to_datetime(
        available_maturities_underlying["DeribitNaming"], format="%d%b%y"
    )
    ...
    available_maturities_underlying = available_maturities_underlying.sort_values(
        by="RealNaming"
    ).reset_index(drop=True)
    ...
    for selected_maturity in available_maturities.DeribitNaming:
        ...
        get_underlying = await send_request(
            get_ticker_by_instrument_request(selected[0]), show_answer=False
        )
        ...
        if "SYN" not in get_underlying:
            ...
            if cfg["raise_error_at_synthetic"]:
                raise ValueError(
                    "Cannot subscribe to order book for synthetic underlying"
                )
            else:
    ...
    full_list.extend(
        [
            f"{currency.currency}-{obj}"
            for obj in available_maturities_underlying.DeribitNaming.values
        ]
    )
...
async def start_scrapper(configuration_path=None):
    ...
    instruments_list = await scrap_all_instruments_from_currency(
